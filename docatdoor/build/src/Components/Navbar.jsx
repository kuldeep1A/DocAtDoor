...
const NavBar = () => {
  ...
  const showNavBar = (event) => {
    ...
    const NavItems = parentDiv.querySelector("[data-items]");
  ...
  return (
    <>
      <div
        ...
        className="w-full h-[100px] shadow-md flex flex-row justify-between items-center px-4"
      >
        ...
        <div className="flex flex-row items-center gap-4">
          <div data-items={"___NavItem"} className={styles.NavItem}>
            <nav>
              <ul className="flex flex-row gap-x-6">
                {NavbarLinks.map((link, index) => (
                  <li
                    key={index}
                    className="text-sky-500 text-lg font-semibold cursor-pointer"
                  >
          ...
          <div data-signinup={"___SignOption"} className={`flex flex-row gap-x-3 ${styles.SignInUp}`}>
            <button
              disabled={loggedIn}
